pipeline {
    ...
    environment {
        //add path to aws cli to the $PATH $ENV so that it will be available
        PATH=sh(script:"echo $PATH:/usr/local/bin", returnStdout:true).trim()
        AWS_REGION = "us-east-1"
        AWS_ACCOUNT_ID=sh(script:'export PATH="$PATH:/usr/local/bin" && aws sts get-caller-identity --query Account --output text', returnStdout:true).trim()
        ECR_REGISTRY="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com"
        APP_REPO_NAME = "lawlkns-repo/phonebook-app"
        APP_NAME = "phonebook"
    ...
    stages {
        ...
        stage('Create Infrastructure for the App') {
            steps {
                ...
                script {
                    ...
                    sh 'aws wait instance-status-ok --instance-ids $id'
                    ...
                    env.MASTER_INSTANCE_PUBLIC_IP = sh(script:'aws ec2 describe-instances --region ${AWS_REGION} --filters Name=tag-value,Values=davids-docker-grand-master Name=instance-state-name,Values=running --query Reservations[*].Instances[*].[PublicIpAddress] --output text | sed "s/\\s*None\\s*//g"', returnStdout:true).trim()  
                }
            }
        }
        ...
        stage('Test the Infrastructure') {
             ...
             steps {
                 ...
                 script {
                 while(true) {
                 //exception handling - testing if docker visualizer is up and running
                 try {
    ...
    post {
        always {
            //delete local images - pipeline will pull images on every run
            echo 'Deleting all local images'
            sh 'docker image prune -af'
        }

        success{
                //wait 5 days for manual approval to destroy infrastructure. After 5 days, destroy will auto run
        ...
        failure {
            //delete ECR Repo - there is NO check in place to see if REPO exists already
